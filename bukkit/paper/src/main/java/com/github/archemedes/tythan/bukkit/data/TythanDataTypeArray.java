...
public class TythanDataTypeArray <T extends ConfigurationSerializable> implements PersistentDataType<byte[], T[]> {
    private final Class<T> type;
    private final Class<T[]> types;

    @NotNull @Override public Class<byte[]> getPrimitiveType() {
        return byte[].class;
    }

    @NotNull @Override public Class<T[]> getComplexType() {
        return types;
    }

    @Override @SneakyThrows public byte @NotNull [] toPrimitive(@NotNull T @NotNull [] serializable, @NotNull PersistentDataAdapterContext persistentDataAdapterContext) {
    ...
    @NotNull @Override @SneakyThrows
    @SuppressWarnings("unchecked") public T @NotNull [] fromPrimitive(byte @NotNull [] bytes, @NotNull PersistentDataAdapterContext persistentDataAdapterContext) {
        try (
